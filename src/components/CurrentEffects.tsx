...
const CurrentEffects = observer(() => {
  ...
  const ranged =
    numericEffectTotals.get("talent_percentage_ranged_damage") ?? 0;
  ...
  if (
    extraAllStyleAccuracy > 0 &&
    !store.currentEffects.has("talent_all_style_accuracy")
  ) {
    currentEffects.push({
    ...
    <div className="flex flex-col w-full">
      ...
      <div className="flex-1 text-xs overflow-y-scroll">
        {currentEffects.length === 0 ? (
          ...
          <ul>
            {currentEffects.map(({ skillTreeNodeInfo, values }, ix) => {
              let effectValue = combineEffectValues(values);

              let extraNote = null;

              if (
                skillTreeNodeInfo.effect.name === "talent_all_style_accuracy"
              ) {
